    ...
    def media_set_title(media_set, with_link = false, with_main_thumb = false, total_thumbs = 0, accessible_resource_ids = nil)
      content = capture_haml do
        ...
        haml_tag :div, :class => div_class do
          ...
          haml_tag :br
          haml_tag :br
          haml_concat " (%d Medieneinträge)" % [media_set.media_resources.count]
          haml_tag :br
          if total_thumbs > 0
            haml_tag :br
      ...
      capture_haml do
        if with_link
          haml_tag :a, content, :href => resource_path(media_set)
        else
          haml_concat content
        end
    ...
    def media_sets_setter(form_path, with_cancel_button = false)
      form_tag form_path, :id => "set_media_sets" do
        ...
        b += content_tag :span, :style => "margin-right: 1em;" do
          select_tag "media_set_ids[]", options_for_select({_("- Auswählen -") => nil}) + options_from_collection_for_select(Media::Set.accessible_by(current_ability, :update), :id, :title_and_user), :style => "width: 100%;"
        end
